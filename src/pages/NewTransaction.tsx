...
export default function NewTransaction() {
  ...
  // 1. TODOS OS 8 ESTADOS ORIGINAIS PRESERVADOS
  ...
  // 🔄 CARGA DINÂMICA: Busca as 15+ categorias do banco
  useEffect(() => {
  ...
  // 🚀 AUTOMAÇÃO: Natureza (Fixo/Variável) muda sozinha ao clicar na categoria
  const handleCategorySelect = (cat: Category) => {
    setCategoryName(cat.name);
    setNature(cat.default_nature); 
  };

  // MÁSCARA DE MOEDA R$ (Idêntica à sua original)
  ...
  // SUBMIT COM TODOS OS CAMPOS (Sem cortes)
    ...
    <div className="min-h-screen bg-gradient-main text-slate-100 font-display flex justify-center no-scrollbar overflow-y-auto selection:bg-primary selection:text-background-dark">
      <div className="relative flex h-full min-h-screen w-full max-w-md flex-col bg-transparent border-x border-white/5">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col px-6 pb-12 space-y-8">
          ...
          <div className="flex h-12 w-full items-center rounded-2xl bg-white/5 p-1.5 border border-white/10">
            {['expense', 'income'].map((t) => (
              <button
                ...
                className={`flex-1 h-full rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${type === t ? 'bg-primary text-background-dark shadow-neon-strong' : 'text-slate-500'}`}
              >
          ...
          {/* Seção Categorias: Água, Energia, IPTU, etc. */}
              ...
              {categories.length > 0 ? (
                categories.map((cat) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 shrink-0 transition-all ${categoryName === cat.name ? 'scale-110 opacity-100' : 'opacity-40'}`}
                  >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex: Conta de Luz" className="w-full glass rounded-2xl py-5 px-6 outline-none focus:border-primary/50 text-sm" />
            </div>

            {/* Responsável (Antigo Pago Por) */}
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Responsável</label>
              <div className="grid grid-cols-3 gap-3">
                {[{ id: 'me', label: 'Eu', icon: 'person' }, { id: 'partner', label: 'Jota', icon: 'favorite' }, { id: 'shared', label: 'Nós', icon: 'group' }].map((item) => (
                  <button key={item.id} type="button" onClick={() => setPaidBy(item.id as any)} className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${paidBy === item.id ? 'bg-primary/10 border-primary text-primary shadow-neon-soft' : 'glass border-white/5 text-slate-500'}`}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-4 gap-2">
                {[{ id: 'pix', label: 'Pix', icon: 'qr_code_2' }, { id: 'debit', label: 'Débito', icon: 'credit_card' }, { id: 'credit', label: 'Crédito', icon: 'payments' }, { id: 'cash', label: 'Dinheiro', icon: 'savings' }].map((item) => (
                  <button key={item.id} type="button" onClick={() => setPaymentMethod(item.id as any)} className={`flex flex-col items-center gap-2 p-3 rounded-xl border transition-all ${paymentMethod === item.id ? 'bg-primary/20 border-primary text-primary' : 'glass border-white/5 text-slate-500'}`}>
